...
#	SVNVERSION=$(echo ${WORKDIR} | sed -nr 's/.*svnr([^.*]+)-.*/\1/p')
	...
	DISTRO_VERSION_OE="${DISTRO_VERSION_OE}"
	echo "DISTRO_VERSION_OE: ${DISTRO_VERSION_OE}"

	DISTRO_NAME="${DISTRO_NAME}"
	echo "DISTRO_NAME: ${DISTRO_NAME}"

	echo "./oealliance.sh ${CACHEDIR} ${KERNELDIR} ${ROOTDIR} ${TYPE} ${SRCDIR} ${CPU} ${STM} ${BOXNAME} ${DISTRO_NAME} ${DISTRO_TYPE} ${SWTYPE} ${IMAGE_NAME} ${GITVERSION} ${SVNVERSION} ${MACHINE_BRAND} ${MACHINE_NAME} ${DRIVERSDATE} ${DISTRO_VERSION} ${DISTRO_TYPE} ${DISTRO_VERSION_OE} ${DISTRO_NAME}"
	./oealliance.sh "${CACHEDIR}" "${KERNELDIR}" "${ROOTDIR}" "${TYPE}" "${SRCDIR}" "${CPU}" "${STM}" "${BOXNAME}" "${DISTRO_NAME}" "${DISTRO_TYPE}" "${SWTYPE}" "${IMAGE_NAME}" "${GITVERSION}" "${SVNVERSION}" "${MACHINE_BRAND}" "${MACHINE_NAME}" "${DRIVERSDATE}" "${DISTRO_VERSION}" "${DISTRO_TYPE}" "${DISTRO_VERSION_OE}" "${DISTRO_NAME}"
