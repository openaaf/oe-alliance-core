...
SVNDIR = "svn/${PN}"
SRC_URI = "svn://svn.dyndns.tv/svn/ipk/source.arm;module=playersnp_vavoo;protocol=http;user=public;pswd=public;externals=allowed"

S = "${UNPACKDIR}/playersnp_vavoo"

FILES:${PN} = "/etc/init.d /python"

do_install() {
    cd ${S}
    cp -a _path_/etc ${D}/
    cp -a _path_/python ${D}/
}
